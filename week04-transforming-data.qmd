...
ggplot(data = penguins_data, aes(x = flipper_length_mm, y = body_mass_g, color = species, shape = island)) +
  ...
  theme_bw() +
  scale_color_viridis_d() +
  # here is where we designate faceting!
   labs(title = "Palmer Penguins, Flipper Length & Body Mass",
       x = "flipper length (mm)",
       y = "body mass (g)")
...
mass_by_flipper <- ggplot(data = penguins_data, aes(x = flipper_length_mm, y = body_mass_g, color = species, shape = island)) +
  ...
  scale_color_viridis_d(option="magma") +
   facet_wrap(~sex) +
   labs(title = "Palmer Penguins, Flipper Length & Body Mass (Male or Female)",
       x = "Flipper Length (mm)",
       y = "Body Mass (g)")
...
ggplot(data = average_flipper_length , aes(x = species , y = average_flipper_length, fill = species )) + # fill in the blanks
  ...
  labs(title = "Flipper Lenght", subtitle = "Flipper", x = "mm", y = "flippy")
...
ex3 <- ggplot(data = penguins_data, aes(x = species, y = culmen_depth_mm, fill = species)) +
  geom_boxplot() +
  theme_dark() + labs(title = "Heavy")

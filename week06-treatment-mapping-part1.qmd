...
Create three basic visualizations using `ggplot2` and the variables you recoded above. Bar plots are a good idea :)

```{r}
freq_p <- ggplot(teds_d, aes(x = fct_infreq(freq1_d))) +
  geom_bar() +
  coord_flip() +
  labs(x = "Frequency of Use of Discharge")

services_p <- ggplot(teds_d, aes(x = fct_infreq(services_d))) +
  geom_bar() +
  coord_flip() +
  labs(x = "Treatement Service")

reason_p <- ggplot(teds_d, aes(x = fct_infreq(reason))) +
  geom_bar() +
  coord_flip() +
  labs(x = "Reason for Discharge Discharge")
